...
type Station = {
  ...
  slug: string;
  canonicalUrl: string;
  urls: string[]; // all original URLs in cluster
...
function stripQueryAndHash(rawUrl: string): string | null {
  try {
...
function toUrlRecords(input: any): UrlRecord[] {
  const out: UrlRecord[] = [];
...
function titleCaseName(s: string): string {
  // Conservative Title Case with some stopwords preserved in lowercase.
...
function normalizeStationName(raw: string): string {
  ...
  // Remove common prefixes/suffixes that are not part of name
  ...
  // Ensure "Police Station" suffix if it looks like a station name without it.
  ...
  if (!hasPoliceStation && (hasPolice || hasStation)) {
    // If it mentions police or station, try to coerce to "... Police Station"
    // e.g. "Watling Street" -> "Watling Street Police Station" (only if URL/title/h1 implied it is a police_station page)
    s = s.replace(/\bpolice\b/i, '').replace(/\bstation\b/i, '');
    s = cleanWhitespace(s);
    if (s.length > 0) s = `${s} Police Station`;
    else s = 'Police Station';
  } else if (!hasPoliceStation) {
    // If it doesn't mention either, still add suffix (since we're only processing police_station pages)
    s = `${s} Police Station`;
  }

  // Clean any doubled suffix
...
function looksLikeUsefulName(s: string): boolean {
...
function extractStationName(rec: UrlRecord): { name: string; extractedFrom: StationSource['extractedFrom'] } | null {
...
function normalizedKey(name: string): string {
  ...
  return cleanWhitespace(name)
    ...
    .replace(/[^a-z0-9 ]+/g, ' ')
    .replace(/\bthe\b/g, ' ')
    .replace(/\bpolice\b/g, ' police ') // keep as anchor token
    .replace(/\bstation\b/g, ' station ') // keep as anchor token
    .replace(/\s+/g, ' ')
    .trim();
}
...
function similarity(aName: string, bName: string): number {
  const a = normalizedKey(aName);
  const b = normalizedKey(bName);
  if (a === b) return 1;
  // Combine token-level and character-level measures
  const tok = jaccardTokens(a, b);
  const tri = jaccardTrigrams(a, b);
  return Math.max(tok, tri);
}

function clusterStations(candidates: { name: string; rec: UrlRecord; extractedFrom: StationSource['extractedFrom'] }[], threshold: number): Station[] {
  // O(n^2) clustering; fine for typical station sets. If it's huge, we can add blocking later.
  const stations: Station[] = [];

  const assign = (cand: (typeof candidates)[number]) => {
    ...
    // Try to merge into existing station clusters by similarity
    let bestIdx = -1;
    let bestScore = -1;
    ...
    if (bestIdx >= 0 && bestScore >= threshold) {
      ...
      // Keep shortest URL as canonical (often cleaner), otherwise keep first
      st.canonicalUrl = [st.canonicalUrl, cand.rec.url].sort((a, b) => a.length - b.length)[0];
      // If slug differs (rare), keep slug from name (stable)
      return;
    }
  ...
  // Finalize: sort URLs within station and sort stations by name
...
async function main(): Promise<void> {
  ...
  // Filter: police_station URLs
  const policeStationRecords = records.filter((r) => r.url.toLowerCase().includes('police_station'));

  const candidates: { name: string; rec: UrlRecord; extractedFrom: StationSource['extractedFrom'] }[] = [];
