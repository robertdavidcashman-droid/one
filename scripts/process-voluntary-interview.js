...
function extractMetadata(html) {
  ...
  return {
    title: document.querySelector('title')?.textContent?.trim() || 'Voluntary Police Interview Solicitor Kent | FREE Legal Advice | PACE Code C',
    description: document.querySelector('meta[name="description"]')?.getAttribute('content') || '',
    canonical: document.querySelector('link[rel="canonical"]')?.getAttribute('href') || 'https://policestationagent.com/voluntary-interviews',
  };
}
...
async function main() {
  ...
  const targetFile = files.find(f => f.includes('Voluntary Police Interview Solicitor Kent') && f.includes('PACE Code C') && f.endsWith('.html'));
  
  if (!targetFile) {
    console.error('❌ Could not find "Voluntary Police Interview Solicitor Kent" HTML file in Downloads');
    process.exit(1);
  }
